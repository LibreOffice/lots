...
public class LocalOverrideStorageStandardImpl implements LocalOverrideStorage
{
   ...
   * @author Matthias Benkmann (D-III-ITD 5.1)
   */
  public class LOSDJDataset implements DJDataset
  {
    /**
     ...
     * Override, die in myBS die (gecachten) Daten aus der Hintergrunddatenbank. myLOS kann null
     * sein, dann wird der Datensatz als nicht aus dem LOS kommend betrachtet.
     */
    protected Map<String, String> myLOS;

    /**
     * Bildet Spaltennamen auf (String-)Werte ab. Die Daten in myLOS repräsentieren den lokalen
     * Override, die in myBS die (gecachten) Daten aus der Hintergrunddatenbank. myBS kann null
     * sein, dann wird der Datensatz als nur aus dem LOS kommend und nicht mit einer
     * Hintergrunddatenbank verknüpft betrachtet.
     */
    protected Map<String, String> myBS;

    /**
     * Die Menge aller Spaltennamen, die dieser Datensatz kennt. Falls dieses Set nicht null ist,
     * werfen Versuche, auf unbekannte Spalten zuzugreifen eine Exception.
     */
    ...
    /**
     * Liefert den Wert, den dieser Datensatz in der Spalte spaltenName stehen hat (override hat
     * vorrang vor backing store). Falls schema nicht null ist, wird eine ColumnNotFoundException
     * geworfen, wenn versucht wird auf eine Spalte zuzugreifen, die nicht im schema ist. Falls
     * weder backing store noch override Speicher einen Wert für diese Spalte haben, so wird null
     * geliefert (nicht verwechseln mit Zugriff auf eine nicht existierende Spalte!).
     */
    @Override
    public String get(String spaltenName) throws ColumnNotFoundException
    {
      if (schema != null && !schema.contains(spaltenName))
        throw new ColumnNotFoundException(L.m("Spalte %1 existiert nicht!", spaltenName));
      String res;
