...
public class MailMerge
{
   ...
   * oder die Datensätze, die der Benutzer in einem Dialog
   * auswählt (offerSelection == false) aus der aktuell über
   * Bearbeiten/Datenbank austauschen eingestellten Tabelle. 
   * Für die Anzeige der Datensätze im Dialog wird die Spalte "WollMuxDescription"
  ...
  private static void mailMerge(XPrintModel pmod, String datasource, String table, Integer type, boolean offerSelection)
  {
     ...
     * Benutzer eine Tabelle auswählt.
     */
   ...
   * Datensätze auswählen kann. Dabei sind alle Datensätze, die eine Spalte "WollMuxSelected"
   * haben, die den Wert "true", "ja" oder "1" enthält bereits vorselektiert.  
   * @param schema muss die Namen aller Spalten für den MailMerge enthalten.
   * @author Matthias Benkmann (D-III-ITD 5.1)
   * TESTED
   */
  ...
  /**
   * Wrapper für ein Dataset, um es einerseits in eine JList packen zu können, andererseits auch
   * dafür, den Zustand ausgewählt oder nicht speichern zu können.
   *
   * @author Matthias Benkmann (D-III-ITD 5.1)
   */
   ...
   *        bestätigt hat. Bei sonstigen Arten, den Dialog zu beenden bleibt das zweite Element
  ...
  private static class SuperMailMerge
  {
     ...
     * Nicht-EDT befüllt, dann aber nur noch im Event Dispatching Thread verwendet bis zu dem
    ...
    /**
     * Der Name der aktuell ausgewählten Tabelle. ACHTUNG! Diese Variable wird initial vom
     * Nicht-EDT befüllt, dann aber nur noch im Event Dispatching Thread verwendet bis zu dem
     * Zeitpunkt wo die Datenquellenauswahl beendet ist und der Druck durch den nicht-EDT
     * Thread angeleiert wird.
     */
    ...
    /**
     * Startet den ultimativen MailMerge. ACHTUNG! Diese Methode kehrt erst zurück, wenn der
    ...
    { //TESTED
      ...
      /*
       * Aktuell über Bearbeiten/Datenbank austauschen gewählte Datenquelle/Tabelle bestimmen,
       * falls gesetzt.
       */
      XTextDocument doc = pmod.getTextDocument();
      XPropertySet settings = null;
    ...
    /**
     * Erstellt die GUI für die Auswahl der Datenquelle/Tabelle für den SuperMailMerge.
     ...
     * @param datasourceAndTableName das erste Element ist der Name der neu ausgewählten
     *        Datenquelle bzw. des Calc-Dokuments. Das zweite Element ist der Name der
     *        vorher ausgewählten Tabelle (oder null). Letzterer wird benötigt, da falls die neue
     ...
     * @param param Parameter, der der Methode übergeben werden soll, oder null falls
    ...
    /**
     * Löscht die {@link #todo}-Liste und fügt ihr dann einen Befehl zum Setzen 
