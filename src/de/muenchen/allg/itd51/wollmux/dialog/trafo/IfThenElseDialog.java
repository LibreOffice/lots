...
public class IfThenElseDialog
{
  ...
  {
    randomImages.add("if.png");
    randomImages.add("else.png");
    randomImages.add("then.png");
    randomImages.add("brown.png");
    randomImages.add("grey.png");
    randomImages.add("ocker.png");
    randomImages.add("orange.png");
    randomImages.add("red.png");
    randomImages.add("navy_blue.png");
    randomImages.add("light_blue.png");
    randomImages.add("lila.png");
    randomImages.add("dark_green.png");
  }
  ...
  private void buildGUI(TrafoDialogParameters params)
  {
    ...
    {
      ConfigThingy resultConf = buildRec(rootNode, new ConfigThingy(""));
      resultConf.setName("Func");
      updateTrafoConf(resultConf); // TODO. validate if
      System.out.println(resultConf.stringRepresentation());
      dialog.endExecute();
    } 
  }
  ...
  private ConfigThingy buildRec(XTreeNode currentNode, ConfigThingy currentConfig) {
    ...
    for (int i = 0; i < currentNode.getChildCount(); i++) {
      ...
      if (WENN.equals(conditionType)) {
        ConfigThingy ifConf = addIf(currentConfig);
        
        addSTRCMPBlock(ifConf, dataChildNode[4], dataChildNode[2], dataChildNode[5]);
  ...
  private XMutableTreeNode createElseNode(String displayValue, String imgFileName)
  {
    ...
    elseNode
        .setNodeGraphicURL(
            Common.class.getClassLoader().getResource("data/" + imgFileName).toString());
  ...
  private AbstractActionListener wennActionListener = event -> {
    ...
    if (selectedNode == null)
    {
      try
      {
        XMutableTreeNode rootNode = createRootNode();
  ...
  private void addNot(ConfigThingy rootNode)
  {
    rootNode.add("NOT");
  }
  ...
  {
      if (conf == null)
        return new ConfigThingy("Func").add("IF");
      else  {
        return conf.add("IF");
      }
  }
  ...
  {
    ConfigThingy catConf = null;
    try
    {
      catConf = rootNode.getByChild("CAT");
  ...
  {
    params.conf = new ConfigThingy(conf.getName());
    params.conf.addChild(conf);
    params.isValid = true;
  }
