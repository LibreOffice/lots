...
public class IfThenElseDialog
{
  ...
  public IfThenElseDialog(List<String> fieldNames, IfThenElseModel model)
  {
    ...
    try
    {
      ...
      dialog = UnoRuntime.queryInterface(XDialog2.class, window);
      ...
      addFinishButtons();
    } catch (UnoHelperException | IllegalArgumentException | Exception e)
    {
      LOGGER.error("", e);
    }
  }

  private void addKeyListener() throws Exception
  {
    XExtendedToolkit extToolkit = UnoRuntime.queryInterface(
        XExtendedToolkit.class, UNO.xMCF.createInstanceWithContext("com.sun.star.awt.Toolkit", UNO.defaultContext));
  ...
  private void deleteCondition()
  {
    ...
    if (selectedNode != null)
    {
      ...
      if (!(node instanceof IfModel))
      {
        ...
        parentNode = UnoRuntime.queryInterface(XMutableTreeNode.class, selectedNode.getParent());
      }
  ...
  private void addTreeControl()
      throws UnoHelperException, Exception
  {
    XTreeControl treeControl = UnoRuntime.queryInterface(XTreeControl.class, controlContainer.getControl(CONTROL_RESULTTREE));
    XControl xTreeControl = UNO.XControl(treeControl);
    XControlModel xControlModel = xTreeControl.getModel();
    UNO.setProperty(xControlModel, "HideInactiveSelection", Boolean.TRUE);
    UNO.setProperty(xControlModel, "RootDisplayed", Boolean.TRUE);
    UNO.setProperty(xControlModel, "RowHeight", 40); // SelectionType?
    XMutableTreeDataModel treeNodeModel = GuiFactory.createTreeModel(UNO.xMCF, UNO.defaultContext);
    UNO.setProperty(xControlModel, "DataModel", treeNodeModel);
    ...
    AbstractMouseListener mouseListener = new AbstractMouseListener()
    {
      ...
      {
        XMutableTreeNode node = UnoRuntime.queryInterface(XMutableTreeNode.class, treeControl.getClosestNodeForLocation(event.X, event.Y));
        try
        {
          nodeSelected(node);
  ...
  private void addNewConditionButton()
  {
    ...
    AbstractActionListener newConditionBtnActionListener = e -> 
    {
      ...
      XMutableTreeNode parent = UnoRuntime.queryInterface(XMutableTreeNode.class, node.getParent());
      int index = parent.getIndex(node);
      UnoRuntime.queryInterface(XTreeControl.class, controlContainer.getControl(CONTROL_RESULTTREE)).clearSelection();
      try
      {
  ...
  {
    XTreeControl treeControl = UnoRuntime.queryInterface(XTreeControl.class, controlContainer.getControl(CONTROL_RESULTTREE));
    return UnoRuntime.queryInterface(XMutableTreeNode.class, treeControl.getSelection());
  }
  ...
  private XMutableTreeNode createNode(String dataValue, String displayValue, String imageURL) throws UnoHelperException
  {
    ...
    XControlModel xControlModel = xTreeControl.getModel();
    XMutableTreeDataModel treeNodeModel = UnoRuntime.queryInterface(
        XMutableTreeDataModel.class, UNO.getProperty(xControlModel, "DataModel"));
  ...
  private void deleteChildrenFrom(XMutableTreeNode node) throws IndexOutOfBoundsException
  {
    ...
    {
      deleteChildrenFrom(UnoRuntime.queryInterface(XMutableTreeNode.class, node.getChildAt(i)));
      ...
      throws IndexOutOfBoundsException, ExpandVetoException
  {
    ...
    UnoRuntime.queryInterface(XTreeControl.class, controlContainer.getControl(CONTROL_RESULTTREE)).expandNode(selectedNode);

    if (selectedNode.getParent() == null)
    {
      selectedNode = UnoRuntime.queryInterface(XMutableTreeNode.class, selectedNode.getChildAt(0));
    }

    UnoRuntime.queryInterface(XTreeControl.class, controlContainer.getControl(CONTROL_RESULTTREE)).expandNode(selectedNode);
    UnoRuntime.queryInterface(XTreeControl.class, controlContainer.getControl(CONTROL_RESULTTREE)).select(selectedNode);
  ...
  private void changeText(String title, String label)
  {
    try
    {
      ...
      UNO.setProperty(frame.getModel(), "Label", title);
